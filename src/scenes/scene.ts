...
export default class Scene extends Phaser.Scene {
  ...
  scaleObject(
    ...
    const scale = this.calcObjectScale(
      objectSize.width,
      objectSize.height,
      availableWidth,
      availableHeight,
  ...
  getObjectSize(
    object: Phaser.GameObjects.GameObject &
      Partial<Phaser.GameObjects.Components.Size>
  ) {
  ...
  calcObjectScale(
    ...
    const widthRatio =
      (width * currentDevicePixelRatio + 2 * padding) / availableWidth;
    const heightRatio =
      (height * currentDevicePixelRatio + 2 * padding) / availableHeight;
